...
export default function AdminProducts() {
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:8000/api/products?currentPage=${currentPage}&sortBy=${sort[0]}&order=${sort[1]}&pageSize=${pageSize}&searchByName=${searchByName}`,
    )
      .then((resp) => resp.json())
  ...
  let increasePage = () => {
    if (pageSize >= ProductList.length) setCurrentPage((c) => c + 1);
